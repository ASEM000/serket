    ...
    def __init__(
        ...
        *,
        ...
        for i, (key, out_dim) in enumerate(zip(keys, layers[1:])):
            ...
            if isinstance(in_dim, Sequence) and isinstance(out_dim, Sequence):
                assert (
            ...
            elif isinstance(in_dim, int) and isinstance(out_dim, Sequence):
                assert (
                    len(out_dim) == layers[-1]
                ), f"Size of sequence must match output node. Found len({out_dim})= {len(out_dim)} != {layers[-1]}"
            ...
            elif isinstance(in_dim, Sequence) and isinstance(out_dim, int):
                if i != (len(layers) - 2):
                    raise ValueError(
